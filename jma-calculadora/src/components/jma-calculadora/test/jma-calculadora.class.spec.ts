...
describe('Pruebas aisladas de la calculadora', () => {
  ...
  let calc: JmaCalculadora;
	beforeAll(() => {
		calc = new JmaCalculadora()
      ...
      describe('Calcula sumas', function () {
        ...
        [0.1, 0.2, 0.3], [9.9, 1.3, 11.2]].forEach(([op1, op2, rslt]) => {
          it(`Suma: ${op1} + ${op2} = ${rslt}`, function () {
            ...
            calc.calcula('+')
            expect(calc.updated.emit).toBeCalledWith(op1);
            calc.ponOperando(op2)
            calc.calcula('=')
            expect(calc.pantalla).toBe(rslt.toString())
            expect(calc.updated.emit).toBeCalledWith(+rslt);
            expect(calc.equaled.emit).toBeCalledWith(+rslt);
          })
        });
      });
